...
[ -f "$INDEX" ] || die "ملف public/index.html غير موجود"

echo "========================================"
echo "   ربط المحفظة الحقيقية داخل شاشة أنا"
echo "========================================"
...
if old not in s:
    raise SystemExit("لم يتم العثور على تعريف wallet المتوقع")

s = s.replace(old, new, 1)

marker = "/* WALLET_ME_CENTER_STYLE */"
...
'''
    if "</style>" not in s:
        raise SystemExit("لم يتم العثور على نهاية style")
...
(function () {
    ...
    function walletDate(value) {
        ...
        if (Number.isNaN(d.getTime())) return walletEscape(value);
    ...
    async function loadRealWallet() {
        ...
        panel.innerHTML = '<div class="wallet-me-empty">جاري تحميل المحفظة...</div>';
        ...
        try {
                        ...
                        <div class="wallet-me-transaction">
                            <div>
                                <div class="wallet-me-transaction-type">
                                    ${walletEscape(
                                        tx.description ||
                                        (credit ? 'إضافة رصيد' : 'خصم من الرصيد')
                                    )}
                                </div>
            ...
            panel.innerHTML = `
                <div class="wallet-me-balance">
                    <div class="wallet-me-balance-label">الرصيد المتاح</div>
            ...
            panel.innerHTML = `
                <div class="wallet-me-error">
                    ${walletEscape(error.message || 'تعذر تحميل المحفظة')}
                </div>
            `;
        }
    }
    ...
    function openWalletFromMe() {
        const existing = document.getElementById('realWalletPanel');
...
'''
    ...
    if "</body>" not in s:
        raise SystemExit("لم يتم العثور على نهاية body")

    s = s.replace("</body>", js + "\n</body>", 1)

# اربط زر المحفظة الحالي
old_action = """data-me-action="wallet" onclick=""""

if old_action in s and "openWalletFromMe()" not in s:
    s = s.replace(
        old_action,
        """data-me-action="wallet" onclick="openWalletFromMe(); return false;" """,
        1
    )
...
echo
echo "========================================"
echo "✅ تم ربط المحفظة داخل شاشة أنا"
echo "========================================"
echo "المسار المستخدم: /api/wallet"
echo "الرصيد: available_minor"
echo "المحجوز: reserved_minor"
echo "المعاملات: wallet_transactions"
echo
echo "⚠️ لم يتم تشغيل WebRTC"
echo "⚠️ لم يتم تنفيذ GitHub"
echo "⚠️ لم يتم تنفيذ Railway"
echo "========================================"

rm -f install-wallet-me.sh
